...
class MemberService(
    ...
    fun checkPassword(member: Member, password: String) {
        if (member.password.isNullOrBlank() || !passwordEncoder.matches(password, member.password)) {
    ...
    fun modifyOrJoin(username: String, password: String?, nickname: String, profileImgUrl: String?): RsData<Member> {
        ...
        return if (existing.isEmpty) {
            ...
            modify(existing.get(), nickname, profileImgUrl)
            RsData("200-1", "회원 정보가 수정되었습니다.", existing.get())
        }
    }
